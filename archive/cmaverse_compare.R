...
Y <- rnorm(n, -3 - 0.4*A - 1.2*M + 0.5*A*M - 0.5*L + 0.3*C1 - 0.6*C2)
...
res_gformula <- cmest(data = data, model = "gformula", outcome = "Y", exposure = "A",
                      ...
                      mreg = list("logistic"), yreg = "linear", postcreg = list("linear"),
                      astar = 0, a = 1, mval = list(0), 
                      estimation = "imputation", inference = "bootstrap", nboot = 100)
summary(res_gformula)

############################################################################################################################################################################################
## Test my mediational g-formula
############################################################################################################################################################################################
data <- as.data.table(data)
path_cb <- fread(paste0(repo, '/cmaverse_dag.csv'))
## Fit all models.
data[, pweight_1 := 1]
data[, id := 1:.N]
...
total_sim_count <- 100 ## Number of gformula simulations (via parametric bootstraps on fitted GLMS).
...
effect_table
## Compare fits to CMAverse for 1 M and 1 L
...
cma_intref <- rnorm(10000, cma_effects[effect=='rintref',mean], cma_effects[effect=='rintref',se])
cma_intmed <- rnorm(10000, cma_effects[effect=='rintmed',mean], cma_effects[effect=='rintmed',se])
...
all_effects <- rbind(effect_table,cma_effects,fill=T)
...
############################################################################################################################################################################################
## Compare multiple INDEPENDENT mediators 
############################################################################################################################################################################################
expit <- function(x) exp(x)/(1+exp(x))
n <- 10000
C1 <- rnorm(n, mean = 1, sd = 0.1)
...
M2 <- rbinom(n, 1, expit(1 + 3*A - L + 1.1*C1 + 2.3*C2))
#Y <- rbinom(n, 1, expit(-3 - 0.4*A - 1.2*M + 0.5*A*M - 0.5*L + 0.3*C1 - 0.6*C2))
...
## Compare fits to CMAverse for 1 M and 1 L
effect_table[, type := 'medgformula']
total_pai <- effect_table[effect %in% c('PAI_M1','PAI_M2'), list(mean=sum(mean)), by=c('type')]
...
############################################################################################################################################################################################
## Compare multiple DEPENDENT mediators 
############################################################################################################################################################################################
expit <- function(x) exp(x)/(1+exp(x))
n <- 10000
C1 <- rnorm(n, mean = 1, sd = 0.1)
...
# formula_m0 <- Y ~ C1 + C2
# formula_m1 <- update(formula_m0, ~ . + L)
# formula_m2 <- update(formula_m1, ~ . + M1)
# formula_m3 <- update(formula_m2, ~ . + M2)
# glm_m0 <- glm(formula_m0, data = data)
# glm_m1 <- glm(formula_m1, data = data)
# glm_m2 <- glm(formula_m2, data = data)
# glm_m3 <- glm(formula_m3, data = data)
# glm_ymodels <- list(glm_m0, glm_m1, glm_m2, glm_m3)
# paths_glm <- paths(a = "A", y = "Y", m = mediators,
#                    glm_ymodels, data = data, nboot = 250)
# summary(paths_glm)
# data(tatar)
...
cma_effects <- data.table(effect=names(res_gformula$effect.pe),
                          ...
                          upper=res_gformula$effect.ci.high,
                          lower=res_gformula$effect.ci.low)
cma_intref <- rnorm(10000, cma_effects[effect=='rintref',mean], cma_effects[effect=='rintref',se]) ## Just for quick comparison
cma_intmed <- rnorm(10000, cma_effects[effect=='rintmed',mean], cma_effects[effect=='rintmed',se]) ## Just for quick comparison
cma_pai <- cma_intref + cma_intmed
cma_pai <- data.table(effect='PAI_M',
                      mean=mean(cma_pai),
                      upper=quantile(cma_pai,0.975),
...
cma_effects[, type := 'CMAverse']
all_effects <- rbindlist(list(effect_table,cma_effects,total_pai),fill=T)
## Plot effects together; sum of separate PAI via M1/M2 adds up to CMAverse joint PAI via M1/M2. 
ggplot(data=all_effects) + 
  geom_hline(yintercept=0,color='black') +
  geom_linerange(aes(x=effect,
                     ymin=lower,
