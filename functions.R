  ...
  for(i in 1:length(formulas)) {
    ...
    if(grepl('_',f_cb[, age])) {
      ...
      rep_data <- function(d,ages) {
        tv_vars_reshape <- unique(unlist(lapply(f_cb[, tv_vars], function(x) unlist(strsplit(x,',')))))
        tv_vars_reshape <- c(unique(gsub('l[.]','',tv_vars_reshape)))
        tv_vars_reshape <- gsub('as[.]factor[(]','',tv_vars_reshape)
        tv_vars_reshape <- gsub('[)]','',tv_vars_reshape)
        d <- d[order(id,age)]
        d[, (paste0('l.',tv_vars_reshape)) := shift(.SD), .SDcols=tv_vars_reshape, by='id']
        if(grepl('wodtke',f_cb[,update_vars])) d <- d[age>=2,]
        d[, env := as.numeric(as.character(env))]
        return(d)
      }
      t_DF_imp_list <- df_long
      t_DF_imp_list[[1]] <- lapply(t_DF_imp_list[[1]], rep_data, ages)
      ## Fit pooled models
      ff.design <- svydesign(id=~id, weights=~pweight, data=t_DF_imp_list)
      models <- gfit.init.survey.imputed(formulas[i], families[i], survey_design=ff.design)
...
multmed_gformula <- function(repo,
  ...
  if(parallelize) {
    ...
    if(windows) {
      ...
      clusterEvalQ(cluster, source(paste0(repo,"/multmed_gcomp_functions.R")))
...
run_gformula_bw_cluster <- function(b, data, outcome_vars=NULL, tc_vars, models, binary_cols=NULL, steps, sim_dir, path_cb, mc_replicates,
  ...
  for(n in names(intervention_rules)) {
    ...
    if(decomp & n!=decomp_compare_df) {
      ...
      decomp_cb <- path_cb[update_vars==o,]
      ...
      if(decomp_type=='4way') {
        ...
        for(path in decomp_paths[[n]][['paths']]) {
          all_paths <- unlist(decomp_paths[[n]][['paths']])
          these_intervention_vars <- path
...
post_process_course <- function(course, total_sims, sim_dir, decomp_paths, decomp_type='4way') {
  ...
  if(dir.exists(paste0(sim_dir,'/',course,'/effects/'))) {
    if(decomp_type=='4way') {
      paths <- c('y1','y2',
                 paste0('y3_',decomp_paths$treatment_course$paths),
                 paste0('y4_',decomp_paths$treatment_course$paths),
                 paste0('y5_',decomp_paths$treatment_course$paths),
                 paste0('y6_',decomp_paths$treatment_course$paths))
      for(path in paths) {
    ...
    if(decomp_type=='2way') {
      for(path in names(decomp_paths[[course]][['paths']])) {
        read_path_sim <- function(x, course, path) {
          batch <- round(x/250)
          readRDS(paste0(sim_dir,'/',course,'/effects/',path,'/',batch,'/sim',x,'.RDS'))
        }
...
multmed_decomp <- function(intervention_course, compare_course, total_sims, sim_dir, decomp_paths, decomp_type='4way') {
  outcome_var <- decomp_paths[[intervention_course]]$outcomes
  paths <- decomp_paths$treatment_course$paths
  ...
  if(decomp_type=='2way') {
    paths <- names(decomp_paths[[intervention_course]]$paths)
    for(path in paths) {
      path_course <- readRDS(paste0(sim_dir,'/', intervention_course, '_', path, '_', total_sims, '.RDS'))
      setnames(path_course, outcome_var, path)
      path_course <- path_course[, c('sim',path), with=F]
